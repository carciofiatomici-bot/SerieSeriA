//
// ====================================================================
// CHALLENGE MINIGAME - Minigioco per Sfide
// ====================================================================
...
(function() {
    ...
    function init() {
        ...
        modal.id = 'challenge-minigame-modal';
        modal.className = 'fixed inset-0 bg-black bg-opacity-90 z-50 hidden flex items-center justify-center';
        modal.innerHTML = `
            <div class="relative w-full h-full flex flex-col items-center justify-center">
                <!-- Bottone Chiudi -->
                <button id="challenge-minigame-close" class="absolute top-4 right-4 z-20 text-white text-3xl hover:text-red-400 transition">
                    <i class="fas fa-times"></i>
                </button>

                <!-- Canvas Container -->
                <div id="challenge-minigame-canvas-container" class="relative">
                    <canvas id="challenge-minigame-canvas"></canvas>

                    <!-- HUD -->
                    <div id="challenge-minigame-hud" class="absolute top-2 left-2 text-white text-sm font-bold hidden" style="text-shadow: 2px 2px 4px rgba(0,0,0,0.8);">
                        <div>Ruolo: <span id="cmg-hud-role"></span></div>
                        <div>Tentativo: <span id="cmg-hud-attempt">1</span>/3</div>
                        <div>Successi: <span id="cmg-hud-score">0</span></div>
                    </div>

                    <!-- Feedback -->
                    <div id="challenge-minigame-feedback" class="absolute inset-0 flex items-center justify-center pointer-events-none opacity-0 transition-opacity duration-300">
                        <span class="text-4xl sm:text-6xl font-black" style="text-shadow: 3px 3px 0 #000;"></span>
                    </div>
                </div>

                <!-- Menu Selezione Ruolo -->
                <div id="challenge-minigame-menu" class="absolute inset-0 flex items-center justify-center bg-black bg-opacity-70">
                    <div class="bg-gray-800 rounded-xl p-6 max-w-md w-11/12 border-2 border-green-500 shadow-2xl">
                        <h2 class="text-2xl sm:text-3xl font-bold text-center text-white mb-2">Training Camp</h2>
                        <p class="text-gray-400 text-center mb-6">Seleziona il tuo ruolo:</p>
                        <div class="space-y-3">
                            <button data-role="gk" class="cmg-role-btn w-full py-3 px-4 bg-gradient-to-r from-yellow-600 to-yellow-700 text-white font-bold rounded-lg hover:from-yellow-500 hover:to-yellow-600 transition transform hover:scale-[1.02] flex items-center justify-center gap-3">
                                <span class="text-2xl">🧤</span> Portiere (Para)
                            </button>
                            <button data-role="def" class="cmg-role-btn w-full py-3 px-4 bg-gradient-to-r from-blue-600 to-blue-700 text-white font-bold rounded-lg hover:from-blue-500 hover:to-blue-600 transition transform hover:scale-[1.02] flex items-center justify-center gap-3">
                                <span class="text-2xl">🛡️</span> Difensore (Intercetta)
                            </button>
                            <button data-role="mid" class="cmg-role-btn w-full py-3 px-4 bg-gradient-to-r from-green-600 to-green-700 text-white font-bold rounded-lg hover:from-green-500 hover:to-green-600 transition transform hover:scale-[1.02] flex items-center justify-center gap-3">
                                <span class="text-2xl">👟</span> Centrocampista (Passa)
                            </button>
                            <button data-role="fwd" class="cmg-role-btn w-full py-3 px-4 bg-gradient-to-r from-red-600 to-red-700 text-white font-bold rounded-lg hover:from-red-500 hover:to-red-600 transition transform hover:scale-[1.02] flex items-center justify-center gap-3">
                ...
                <!-- Game Over -->
                <div id="challenge-minigame-gameover" class="absolute inset-0 flex items-center justify-center bg-black bg-opacity-70 hidden">
                    <div class="bg-gray-800 rounded-xl p-6 max-w-md w-11/12 border-2 border-green-500 shadow-2xl text-center">
                        <h2 class="text-3xl font-bold text-white mb-2">Fine Sessione!</h2>
                        <p class="text-xl text-gray-300 mb-4">Punteggio: <span id="cmg-final-score" class="font-bold text-green-400"></span>/3</p>
                        <div class="space-y-3">
                            <button id="cmg-btn-retry" class="w-full py-3 px-4 bg-gradient-to-r from-green-600 to-green-700 text-white font-bold rounded-lg hover:from-green-500 hover:to-green-600 transition">
                                🔄 Riprova
                            </button>
                            <button id="cmg-btn-menu" class="w-full py-3 px-4 bg-gradient-to-r from-gray-600 to-gray-700 text-white font-bold rounded-lg hover:from-gray-500 hover:to-gray-600 transition">
                                📋 Cambia Ruolo
                            </button>
                            <button id="cmg-btn-close" class="w-full py-3 px-4 bg-gradient-to-r from-red-600 to-red-700 text-white font-bold rounded-lg hover:from-red-500 hover:to-red-600 transition">
                                ✕ Chiudi
                            </button>
                        </div>
                    </div>
        ...
        canvas = document.getElementById('challenge-minigame-canvas');
        ctx = canvas.getContext('2d');
        hudEl = document.getElementById('challenge-minigame-hud');
        feedbackEl = document.getElementById('challenge-minigame-feedback');
        menuEl = document.getElementById('challenge-minigame-menu');
        gameOverEl = document.getElementById('challenge-minigame-gameover');

        // Event Listeners
        setupEventListeners();

        console.log('[ChallengeMinigame] Inizializzato');
    }

    function setupEventListeners() {
        // Chiudi modal
        document.getElementById('challenge-minigame-close').addEventListener('click', close);

        // Selezione ruolo
        document.querySelectorAll('.cmg-role-btn').forEach(btn => {
        ...
        // Game Over buttons
        document.getElementById('cmg-btn-retry').addEventListener('click', () => {
            startGame(state.role);
        });
        document.getElementById('cmg-btn-menu').addEventListener('click', showMenu);
        document.getElementById('cmg-btn-close').addEventListener('click', close);
    ...
    function open(options = {}) {
        ...
        console.log('[ChallengeMinigame] Aperto', { testMode: state.testMode });
    }
    ...
    function close() {
        ...
        console.log('[ChallengeMinigame] Chiuso con score:', state.score);
    }
    ...
    function resizeCanvas() {
        ...
        const container = document.getElementById('challenge-minigame-canvas-container');
    ...
    function setupMidfielder(speedMult) {
        ...
        for (let i = 0; i < 3; i++) {
            targets.push({
                ...
                radius: 25,
                vx: (Math.random() > 0.5 ? 1 : -1) * 2.5 * speedMult,
                vy: 0,
                type: 'teammate'
            });
        }
    ...
    function endGame() {
        ...
        document.getElementById('cmg-final-score').textContent = state.score;

        console.log('[ChallengeMinigame] Game Over - Score:', state.score);
    }
    ...
    function updateHud() {
        ...
        document.getElementById('cmg-hud-role').textContent = roles[state.role] || '';
        document.getElementById('cmg-hud-attempt').textContent = Math.min(state.attempts + 1, config.MAX_ATTEMPTS);
        document.getElementById('cmg-hud-score').textContent = state.score;
    }
    ...
    console.log('[OK] Modulo ChallengeMinigame caricato.');
